...
> 编写日期：2026-09-16。
> 与 [`macos-platform.md`](./macos-platform.md) 记录的那一层不同，
...
> **2026-09-16 补记**：macOS 随后也提升为交付目标（`macos-platform.md` §1），
> 本节说的是**当时**两者的差别。现在三个平台的质量门槛一致，区别只剩下各自
> 缺什么；「macOS 那边可以整项不做」这句话对新增功能已经不再成立。

macOS 适配当初的目的是缩短开发回路，取不到的数据可以整项不做（IOKit 的磁盘 IO 就没做，
理由写的是「成本与联调收益不匹配」）。Windows 从一开始就不能这么算账：它是有人会拿来
跑生产的平台，一项缺失意味着那台机器上的运维少一块视野。
...
* `macos-platform.md` 记的是 macOS 那一层。它当初是开发平台，2026-09 也提升为
  交付目标，取舍标准自此与本文一致（见 §1 的补记）。
