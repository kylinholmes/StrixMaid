...
| `GET /files/icon?…` | 按文件类型取图标，返回 PNG。**前端只认这一个接口**，图标来自系统还是外置包还是内置兜底，由后端决定，前端不感知 |
...
| 平台 | 来源 |
|---|---|
| Windows | `SHGetFileInfoW`。仓库已有图标基建（PNG 编码、缓存、预热），接得上 |
| macOS | `NSWorkspace.icon(forFile:)`。需要 objc 互操作，仓库目前**一处 objc 桥都没有** |
| Linux | **外置主题包**，见下；没装包时用内置极简图标 |

**图标包外置，不打进二进制。** 二进制里只放一套我们自己的、与设计语言无关的极简
图标（十来个 SVG，几 KB）。Breeze / Yaru 这类做成独立包，安装时或运行时从 URL 拉；
**拉不到就明说，让人手动放，不静默降级**（与 `11-multi-host.md` §5 对产物的口径一致）。

外置之后 15 MiB 的体积门槛不再是约束，但换来两个新问题，见 §8 未决 1 与 2。
...
1. **图标包选型与许可证。** Breeze 是 LGPL/GPL、Yaru 是 CC-BY-SA 4.0，**再分发要
   满足署名与相应义务**。已另行调研，结论出来后由项目负责人拍板。这条现在花十分钟
   很便宜，发布之后再改很贵。

2. **外置图标包的完整性校验。** 下载下来的东西要渲染进用户界面，来源被替换掉不是
   小事。倾向发布时记录哈希、下载后校验。未定。

3. **复用系统当前的图标主题。** 比自带一套更贴合用户桌面，但要读 icon theme 规范
   并逐级回退，且装了什么主题因发行版而异。放到后续。

4. **Linux 图标覆盖不全时的表现。** 外置包若只覆盖部分 MIME 类型，是混着显示
   （一部分真图标、一部分通用图标）还是整体退回通用？混着显示会很难看，倾向后者，未定。
