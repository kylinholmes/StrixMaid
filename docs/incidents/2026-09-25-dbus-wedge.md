...
## 6. 修复验证（2026-09-25，原机）

修复（#17/#18/#19）部署回**发生事故的那台机器**后，在原触发条件下验证：

- **按需订阅**：服务启动 45 秒无人订阅，零监听日志、bus socket Recv-Q 全 0
  ——没人看时确实不再碰总线信号。
- **压力复现**：以真实 PAM 登录 + 真实 WS 订阅（`services.changed`）保持
  活跃，`dbus-wedge-stress.sh` 灌 6 轮 daemon-reload + 批量 unit 启停
  （正是当初把队列灌满的那类信号洪流）。结果：**Recv-Q 峰值 0、结束 0**
  （事故特征是固定 47182 字节从不下降）、dbus 零拒绝日志、`systemctl`
  全程 5 ms，且 **27 帧事件端到端到达 WS 客户端**——信号 → 监听 →
  冲刷 → 广播 → 客户端全链路通。
- **退场**：订阅断开后监听于下一个 30 秒检查点准点退场（两次断开、
  两次退场，日志一一对应）。
- 期间脚本的前提门禁还真实拦截过一次无效压测（订阅方客户端崩溃、
  监听已退场的状态下拒绝施压）——门禁本身也被验证了。

结论：死锁在原条件下不再复现，按需订阅与退场按设计工作。
