...
//!
//! 薄壳：参数校验交给 types 的 DTO 与 core 的 `validate_unit_name`，业务全在
//! `strixmaid_core::providers::service`。写操作**当前没有权限体系**——认证接线由
//! 另一处完成，这里直接调 provider；非 root 会被 polkit 拒绝，core 已把它映射成
//! `ErrorCode::PermissionDenied` + 「需要管理访问」。
//!
...
use strixmaid_core::providers::service::{ServiceProvider, UnitDeps as CoreUnitDeps};
...
/// 服务路由的状态。`provider` 为 `None` 表示本机没有 systemd（bus 与 systemctl 都不可用），
/// 所有端点返回 501 `capability_unavailable{systemd}`，前端据此隐藏服务页。
pub struct ServicesState {
    pub provider: Option<Arc<dyn ServiceProvider>>,
}

impl ServicesState {
    pub fn new(provider: Option<Arc<dyn ServiceProvider>>) -> Self {
        Self { provider }
    }

    fn provider(&self) -> Result<&dyn ServiceProvider, ApiErr> {
        self.provider.as_deref().ok_or_else(|| {
            ApiError::capability_unavailable("systemd", "本机没有可用的 systemd").into()
        })
    }
}

/// 构建服务路由。挂到 `/api/v1` 之下（路径已含 `/services` 前缀）。
pub fn router(state: Arc<ServicesState>) -> OpenApiRouter<()> {
    OpenApiRouter::new()
...
pub struct ScopeQuery {
    /// 作用域，缺省 `system`。`user` 只对本进程 uid 的 user manager 有效。
    #[serde(default)]
...
pub async fn list_units(
    State(state): State<Arc<ServicesState>>,
    Query(query): Query<UnitListQuery>,
) -> ApiResult<Json<Vec<UnitSummary>>> {
    Ok(Json(state.provider()?.list_units(&query).await?))
}
...
pub async fn unit_detail(
    ...
    Ok(Json(state.provider()?.unit_detail(scope, &unit).await?))
}
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 403, description = "文件不可读", body = ApiError),
        (status = 404, description = "unit 不存在", body = ApiError),
        (status = 501, description = "本机没有 systemd", body = ApiError),
    ),
)]
pub async fn unit_file(
    ...
    let scope = q.scope.unwrap_or_default();
    Ok(Json(state.provider()?.unit_file(scope, &unit).await?))
}
...
pub async fn unit_deps(
    ...
    let scope = q.scope.unwrap_or_default();
    Ok(Json(
        state.provider()?.unit_deps(scope, &unit).await?.into(),
    ))
}
...
/// 并触发 `daemon-reload`。授权由 polkit 裁决，被拒返回 403 `permission_denied` + 需要管理访问。
#[utoipa::path(
    post,
    ...
    responses(
        ...
        (status = 403, description = "polkit 拒绝，需要管理访问", body = ApiError),
        (status = 404, description = "unit 不存在", body = ApiError),
...
pub async fn unit_action(
    ...
    Ok(Json(
        state
            .provider()?
            .unit_action(scope, &unit, req.action)
            .await?,
    ))
}
